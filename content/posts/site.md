...
I wanted a personal site that did two things: act as a living resume and give me somewhere to write. Most personal sites do one or the other. I wanted both on the same domain, with a consistent look, and without a hosting bill.

The resume page has a chatbot on it. That was a deliberate choice, not a novelty — if someone's looking at my resume and wants to ask whether I've worked with Kubernetes or led a platform team, they can just ask rather than scanning bullet points. It also gave me something interesting to build.
...
The `/resume/` page is different. It uses a completely custom layout (`layouts/resume/single.html`) that renders a two-column container. PaperMod's styles don't apply here — the resume page loads its own `resume.css` independently.

```
content/
├── _index.md          ← home (profile mode)
├── resume/
│   └── index.md       ← resume markdown
└── posts/
    └── *.md           ← blog posts
...
The Worker is stateless — no conversation history. Each request is independent. The resume text is a hardcoded constant in the Worker; updating the resume requires redeploying with `wrangler deploy`.

The LLM prompt instructs the model to answer only from the resume content and decline questions it can't answer from that context. Temperature is 0.3, max tokens 400 — enough for a useful answer, not enough to ramble.
